...
pub fn hint_is_zero(
    ...
    let key_leading_zeroes_nibbles: Felt252 =
        utils::get_value("key_leading_zeroes_nibbles", vm, hint_data)?;

    utils::write_value(
        "is_zero",
        match nibble_index.cmp(&key_leading_zeroes_nibbles) {
...
pub fn hint_nibble_from_low(
    ...
    let get_nibble_from_low =
        if (Felt252::ZERO <= nibble_index && nibble_index <= FELT_31 && key_nibbles <= FELT_32)
...
pub fn hint_needs_next_word(
    ...
    let needs_next_word = if n_bytes > avl_bytes_in_word {
        Felt252::ONE
    } else {
        Felt252::ZERO
    };
...
pub fn hint_words_loop(
    ...
    let n_words_to_handle_in_loop: Felt252 =
        utils::get_value("n_words_to_handle_in_loop", vm, hint_data)?;
